...
func (r *PostgresRepository) Create(ctx context.Context, schedule model.Schedule) error {
	...
	`, schedule.ID, schedule.ProjectID, schedule.SpiderID, schedule.SpiderVersion, schedule.RegistryAuthRef, schedule.Name, schedule.CronExpr, schedule.Enabled, schedule.Image, string(commandJSON), schedule.RetryLimit, schedule.RetryDelaySeconds, schedule.CreatedAt, lastMaterializedAt)
	return err
}
...
func (r *PostgresRepository) List(ctx context.Context) ([]model.Schedule, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&schedule.ID, &schedule.ProjectID, &schedule.SpiderID, &schedule.SpiderVersion, &schedule.RegistryAuthRef, &schedule.Name, &schedule.CronExpr, &schedule.Enabled, &schedule.Image, &commandJSON, &schedule.RetryLimit, &schedule.RetryDelaySeconds, &schedule.CreatedAt, &lastMaterializedAt); err != nil {
			return nil, err
		}
